...
describe("github-issues", function() {
  ...
  let defineModel = function(name, schemaProperties, pluginOptions) {
    ...
    let schema =
      schemaProperties instanceof Schema
        ? schemaProperties
  ...
  describe("A documents with non-schema properties set to hidden", function() {
    it("should hide the properties", function(done) {
      ...
      user.save(function(err, savedUser) {
        ...
        let User2 = mongoose.model("User", User.schema, undefined, {
          cache: false
        });
  ...
  describe("Bug: subDocument returns unpopulated if its not found", function() {
    it("should return empty array", function(done) {
      ...
      Host.create({ ipAddress: "192.168.1.1", status: "OFFLINE" }).then(
        host => {
  ...
  describe("Bug: elements of array is visible", function() {
    it("password should be hidden", function(done) {
      ...
      Host.create({
        name: "Main",
        credentials: [{ login: "root", password: "123456" }]
      }).then(() => {
        Host.findOne({ name: "Main" })
          .then(host => {
  ...
  describe("Bug: array of ref is visible", function() {
    it("tags should be hidden with hide options set out of type option", function(done) {
      ...
      PostSchema.plugin(mongooseHidden, { applyRecursively: true });
      const Post = mongoose.model("Post", PostSchema);

      Post.create({
        title: "Test",
        tags: [{ name: "js" }, { name: "mongoose" }]
      }).then(() => {
        Post.findOne({ title: "Test" })
          .then(post => {
            const obj = post.toObject();
            obj.should.not.have.property("tags");
            done();
          })
          .catch(done);
      });
  ...
  describe("Bug: array of ref is visible", function() {
    it("tags should be hidden with hide options set in of type option", function(done) {
      const TagSchema = new mongoose.Schema(
        {
          name: String
        },
